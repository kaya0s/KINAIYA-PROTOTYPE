...
import type { AnalyzeResponse, Intervention, MasteryCheckResponse } from "@/lib/kinaiyaApi";
...
const promoteLevel = (level: AnalyzeResponse["level"]) => {
  if ((level as string) === "Frustration" || level === "Frustrational") return "Instructional";
  if (level === "Instructional") return "Independent";
  return "Independent";
};

const masteryCheckMock = (questions: string[], studentAnswers: string[]): MasteryCheckResponse => {
  ...
  const nextAction: MasteryCheckResponse["next_action"] =
    masteryAchieved ? "promote" : pct >= 40 ? "repeat_different" : "repeat_simpler";
  ...
  return {
    ...
    question_results: questions.map((q, i) => {
      ...
      return {
        ...
        student_answer: a,
        is_correct: ok,
        feedback: ok ? "Good effort. Keep going." : "Try answering with at least one word or phrase.",
      };
    }),
...
const regenerateInterventionMock = (args: {
  ...
  const order: Intervention["exercise_type"][] = ["fill_blank", "word_match", "reread", "phonics_drill"];
  const idx = Math.max(0, order.indexOf(args.previous.exercise_type));
  const nextType = args.nextAction === "repeat_simpler" ? "reread" : order[(idx + 1) % order.length];
    ...
    nextType === "reread"
      ...
      : nextType === "phonics_drill"
        ...
        : nextType === "word_match"
          ...
          : base.length ? base : ["Maria ___ her plants.", "A tiny ___ grew.", "The sprout became a ___ ."];
...
const Lessons = () => {
  ...
  const [answers, setAnswers] = useState<string[]>(() => intervention?.mastery_check_questions?.map(() => "") ?? []);
  ...
  const adaptiveRationale = useMemo(() => {
    ...
    if (gap.includes("omission")) return "AI detected word skipping patterns. Focus: Foundational Tracking.";
    if (gap.includes("substitution")) return "AI detected decoding errors. Focus: Phoneme Blending.";
    if (gap.includes("vowel")) return "AI detected vowel confusion. Focus: Short/Long Vowel Phonics.";
    if (gap.includes("fluency")) return "AI detected pace struggles. Focus: Speed & Expression.";

    return "Personalized pathway based on MSU Framework analysis.";
  }, [analysis]);

  useEffect(() => {
    if (!intervention) return;
    localStorage.setItem(CURRENT_INTERVENTION_KEY, JSON.stringify(intervention));
  }, [intervention]);
  ...
  const submitMasteryCheck = async () => {
    ...
    try {
      ...
      const res = masteryCheckMock(intervention.mastery_check_questions, answers);
      ...
      try {
        ...
        if (session?.classCode && session?.studentId) {
          const payload = {
            ...
            intervention,
            masteryCheck: { answers, result: res, questions: intervention.mastery_check_questions },
      ...
      setError(e instanceof Error ? e.message : "Could not check mastery. Try again.");
    } finally {
      setChecking(false);
    }
  };
  ...
  const regenerate = async () => {
    ...
    if (result.next_action !== "repeat_different" && result.next_action !== "repeat_simpler") return;
    ...
    try {
      const next = regenerateInterventionMock({ previous: intervention, nextAction: result.next_action });
      setIntervention(next);
      setResult(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not regenerate a new exercise.");
    } finally {
  ...
  const modules = useMemo(() => {
    ...
    const desc = analysis?.gap ? `Focus: ${analysis.gap}` : "Based on your last diagnostic";
    return [
      {
        id: 1,
        title,
  ...
  return (
    <div className="mobile-container bg-background px-5 pb-8">
      <div className="flex items-center gap-3 pt-6 pb-4">
        ...
        <div>
          <h1 className="font-display font-bold text-foreground text-lg">My Learning Path</h1>
          <p className="text-xs text-muted-foreground">Personalized for your reading level</p>
        </div>
      </div>
        ...
        <>
          ...
          >
            <div className="flex items-center gap-3 mb-3 pb-2 border-b border-kinaiya-purple/10">
              ...
              <div>
                <p className="text-xs font-bold text-foreground">Kinaiya AI Insight</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest leading-none">Closed-Loop IPO Logic</p>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-2">
              <div className="space-y-1">
                <p className="text-[8px] text-muted-foreground font-bold uppercase">Input</p>
                <p className="text-[10px] text-foreground font-medium leading-tight">
                  {analysis?.gap.split(":")[0] || "Analysis"}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-[8px] text-muted-foreground font-bold uppercase">Process</p>
                <p className="text-[10px] text-foreground font-medium leading-tight">
                  MSU Adaptation
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-[8px] text-muted-foreground font-bold uppercase">Output</p>
            ...
            {modules.map((mod, i) => (
              ...
              >
                <button
                  ...
                  className={`w-full text-left rounded-2xl border p-4 transition-all ${mod.status === "locked"
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <p className="font-display font-bold text-foreground text-sm">{mod.title}</p>
                    ...
                    >
                      <div className="p-4 mt-2 rounded-2xl bg-card border border-border space-y-4">
                        <div className="rounded-xl bg-muted/40 p-3">
                          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Instructions</p>
                          <p className="text-sm text-foreground mt-1 leading-relaxed">{intervention.instructions}</p>
                        </div>

                        <div className="space-y-2">
                          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Practice</p>
                          {intervention.content.map((item, j) => (
                            <div key={`${item}-${j}`} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50">
                        ...
                        <div className="space-y-2">
                          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Mastery Check</p>
                          {intervention.mastery_check_questions.map((q, idx) => (
                            <div key={q} className="space-y-2 rounded-xl bg-muted/50 p-3">
                          ...
                          {result && (
                            <div className={`rounded-xl border p-3 ${result.mastery_achieved ? "bg-kinaiya-green-light border-kinaiya-green/20" : "bg-kinaiya-blue-light border-kinaiya-blue/20"}`}>
                              <p className="text-sm font-bold text-foreground">
                                {result.mastery_achieved ? "Mastery achieved!" : "Keep going!"}{" "}
                                <span className="text-xs text-muted-foreground font-normal">({result.score})</span>
                              </p>
                              <p className="text-sm text-muted-foreground mt-1 leading-relaxed">{result.overall_feedback}</p>
                              <div className="mt-3 space-y-2">
                                {result.question_results.map((r) => (
                                  <div key={r.question} className="text-xs text-muted-foreground">
                                    <span className="font-medium text-foreground">{r.is_correct ? "Correct:" : "Try again:"}</span>{" "}
                                    {r.feedback}
                                  </div>
                                ))}
                              </div>
                          ...
                          <div className="flex gap-2 pt-1">
                            <button
                              ...
                              disabled={checking || answers.some((a) => !a.trim())}
                            ...
                            >
                              {checking ? "Checking..." : "Submit Mastery Check"}
                            </button>
                            <button
                          ...
                          {result && !result.mastery_achieved && (result.next_action === "repeat_different" || result.next_action === "repeat_simpler") && (
                            <button
                              onClick={regenerate}
